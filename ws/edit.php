...
{ 
	...
	?>
	<form action="http://localhost/ws/index.php?action=edit" method="POST">
	 ...
	 <input type="submit" name="envoyer">Modifier</input>
	</form>
	<?php
}
else
...
$resultat=$clientContacts->edit($_POST['id'],$_POST['name'], $_POST['username'], $_POST['email'],
...
$messageJson->error="Une erreur au niveau de la requête SQL, vérifiez vos données SVP.";
...

	{
		$messageJson->status=308;
		$messageJson->data=$resultat;
		$messageJson->error="Le champs ID est obligatoire pour modifier un contact.";
	}
}
else
{
$messageJson->status=309;
